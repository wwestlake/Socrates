...
#endif
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, exp_ast&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const exp_ast& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yytype)
    {
      case 12: // expression
        value.template destroy< exp_ast > ();
        break;
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == token::END || tok == token::MODULE || tok == token::SEMICOLON || tok == token::OBRACE || tok == token::CBRACE);
      }
#else
      ...
      symbol_type (int tok, std::string v, location_type l)
        : super_type(token_type (tok), std::move (v), std::move (l))
      {
        YY_ASSERT (tok == token::OP || tok == token::IDENT);
      }
#else
    ...
    SOC_Parser (SOC_Scanner  &scanner_yyarg, SOC_Driver  &driver_yyarg);
    virtual ~SOC_Parser ();
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      {
        return symbol_type (token::OP, std::move (v), std::move (l));
      }
#else
      static
      symbol_type
      ...
      {
        return symbol_type (token::OBRACE, std::move (l));
      }
#else
      static
      symbol_type
      ...
      {
        return symbol_type (token::OBRACE, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      {
        return symbol_type (token::CBRACE, std::move (l));
      }
#else
      static
      symbol_type
      ...
      {
        return symbol_type (token::CBRACE, l);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
  ...
  private:
    ...
    SOC_Parser (const SOC_Parser&);
    SOC_Parser& operator= (const SOC_Parser&);
